...
class QBittorrentAppBarStats extends StatelessWidget {
  const QBittorrentAppBarStats({
    Key? key,
  }) : super(key: key);

  @override
  Widget build(BuildContext context) =>
      Selector<QBittorentState, Tuple5<bool, String, String, String, String>>(
  ...
  Future<void> _onTap(BuildContext context, String speed) async {
    ...
    List values = await QBittorrentDialogs.speedLimit(context, speed);
    if (values[0])
      switch (values[1]) {
        case -1:
          {
            values = await QBittorrentDialogs.customSpeedLimit(context);
            if (values[0])
